...
import com.json.netty.util.UserPojoReq;
...
public class ServerRoute extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		restConfiguration().apiContextPath("/openapi.json").component("restlet").contextPath("/dukcapil").port("7070")
		...
		from("direct:appCreated")
				...
				.unmarshal().json(JsonLibrary.Gson, UserPojoReq.class).log("Body : (${body})")
				...
				.bean("generateFullInfo",
						"setValue(${body.getNIK},${body.getUser_id},${body.getPassword},${body.getIp_user})");
				// marshall for generating json as return based on a POJO
				//.marshal().json(JsonLibrary.Gson, UserPojo.class).log("Final Body : ${body}");
	}
